function AssetLoader()
{
    ...
    this.maxAttempts = 5;
    this.loadingRequests    = {scripts: 0, json: 0, text: 0, images: 0, audio: 0, fonts: 0};
    this.loadingSuccess     = {scripts: 0, json: 0, text: 0, images: 0, audio: 0, fonts: 0};
    this.loadingErrors      = {scripts: 0, json: 0, text: 0, images: 0, audio: 0, fonts: 0};
    this.loadingFailed      = {scripts: 0, json: 0, text: 0, images: 0, audio: 0, fonts: 0};
        ...
        if (this.loadingStatus[name] == 'ok' || this.loadingStatus[name] == 'loading')
        {
            if (!forceReload)
            {
                if (this.loadingStatus[name] == 'ok')
                {
                    setTimeout(function()
                    {
                        !dontExecute && eval.call(window, self.loadedScripts[name]);
                        callback && callback(self.loadedScripts[name], name);
                    }, 0);
                }
                return;
            }
        ...
        if (this.loadingStatus[name] == 'ok' || this.loadingStatus[name] == 'loading')
        {
            if (!forceReload)
            {
                if (this.loadingStatus[name] == 'ok')
                {
                    setTimeout(function()
                    {
                        objectToStoreData && (objectToStoreData.data = self.loadedJson[name]);
                        callback && callback(self.loadedJson[name], name);
                    }, 0);
                }
                else
                {
                    ...
                    {
                        this.pendingCallbacks[name] = {success: [callback], error: [errorCallback]}
                    }
                    else
                    {
                        callback && this.pendingCallbacks[name].success.push(callback);
                        errorCallback && this.pendingCallbacks[name].error.push(errorCallback);
                    }
                }
                return;
            }
    ...
    this.loadText = function(name, objectToStoreData, callback, forceReload, errorCallback)
    {
        if (this.loadingStatus[name] == 'ok' || this.loadingStatus[name] == 'loading')
        {
            if (!forceReload)
            {
                if (this.loadingStatus[name] == 'ok')
                {
                    setTimeout(function()
                    {
                        objectToStoreData && (objectToStoreData.data = self.loadedText[name]);
                        callback && callback(self.loadedText[name], name);
                    }, 0);
                }
                else
                {
        ...
        wade.ajax({
            ...
            dataType: 'text',
            timeout: 15000,
            success: this.textLoaded(name, objectToStoreData, callback),
            error: this.textLoadingError(name, objectToStoreData, callback, forceReload, errorCallback)
        });
    };
	...
	this.loadAppScript = function(name, forceReload)
	{
		...
		wade.ajax({
			...
			success: function(data)
            {
                ...
                {
                    wade.error('Main app script ' + name + ' is not a valid JavaScript file: ' + e);
                    that.appLoadingError(name);
                    return;
                }
                that.appLoaded(name)();
            },
    ...
    this.loadImage = function(name, callback, errorCallback)
    {
        ...
        if (this.loadingStatus[name] == 'ok')
        {
            if (callback)
            {
                setTimeout(function()
                {
                    callback(self.loadedImages[name], name);
                }, 0);
            }
            return;
        }
        else if (this.loadingStatus[name] == 'loading')
        {
            ...
            {
				callback && this.loadedImages[name].addEventListener('load', callback, false);
				errorCallback && this.loadedImages[name].addEventListener('error', errorCallback, false);
				return;
            }
        }
        loadListener = this.imageLoaded(name, callback);
        errorListener = this.imageLoadingError(name, callback, errorCallback);
        this.loadingStatus[name] = 'loading';
        this.updateAttempts(name, 'images');
        var image = new Image();
        this.loadedImages[name] = image;
        loadListener && image.addEventListener('load', loadListener, false);
        errorListener && image.addEventListener('error', errorListener, false);
        image.callbackIsSet = !!callback;
        image.src = name;
    };
    ...
    this.imageLoadingError = function(name, callback, errorCallback)
    {
        ...
        return function()
        {
            ...
            if (that.attemptsCount[name] < that.maxAttempts)
            {
                ...
                {
                    wade.error('Unable to load image ' + name);
                }
    ...
    this.setText = function(name, data)
    {
        this.loadedText[name] = data;
        if (data)
        {
            this.loadingStatus[name] = 'ok';
        }
	...
	this.textLoaded = function(name, objectToStoreData, callback)
	{
        var that = this;
		return function(data)
		{
            that.loadingSuccess.text++;
            that.loadingStatus[name] = 'ok';
            that.loadedText[name] = data;
            // if we're preloading text, notify wade so it can pass the text data to the main asset loader
            if (window['wade'])
            {
                wade.setText(name, that.loadedText[name]);
            }
			objectToStoreData && (objectToStoreData.data = data);
            that.handleLoadingCallback(callback, that.loadedText[name], name);
		}
	};
    ...
    this.scriptLoadingError = function(name, callback, forceReload, errorCallback)
    {
        ...
        return function()
        {
            ...
            if (that.attemptsCount[name] < that.maxAttempts)
            {
                ...
                {
                    wade.error('Unable to load script ' + name);
                }
    ...
    this.jsonLoadingError = function(name, objectToStoreData, callback, forceReload, errorCallback)
    {
        ...
        return function()
        {
            ...
            if (that.attemptsCount[name] < that.maxAttempts)
            {
                that.handleErrorCallback(name, errorCallback, 'Unable to load json file')
    ...
    this.textLoadingError = function(name, objectToStoreData, callback, forceReload, errorCallback)
    {
        var that = this;
        return function()
        {
            that.loadingErrors.text++;
            that.loadingStatus[name] = 'error';
            if (that.attemptsCount[name] < that.maxAttempts)
            {
                that.handleErrorCallback(name, errorCallback, 'Unable to load text file');
                that.loadingFailed.text++;
            }
            else
            {
                that.loadText(name, objectToStoreData, callback, forceReload, errorCallback);
            }
        }
    };
	...
	this.appLoadingError = function(name)
	{
        ...
        return function()
        {
            ...
            if (that.attemptsCount[name] < that.maxAttempts)
            {
                wade.error('Unable to load main app script ' + name);
                that.loadingFailed.scripts++;
            }
            else
            {
    ...
    this.audioLoadingError = function(name, autoplay, loop, callback, errorCallback)
    {
        ...
        return function()
        {
            ...
            if (that.attemptsCount[name] < that.maxAttempts)
            {
                ...
                {
                    wade.error('Unable to load audio ' + name);
                }
    ...
    this.isFinishedLoading = function()
	{
        return (this.loadingRequests.scripts    == this.loadingSuccess.scripts  + this.loadingFailed.scripts  &&
                ...
                this.loadingRequests.text       == this.loadingSuccess.text     + this.loadingFailed.text     &&
    ...
    this.getImage = function(name, errorMessage)
    {
        ...
        else
        {
            ...
            errorMessage && wade.warn(errorMessage);
    ...
    this.getJson = function(name)
    {
        ...
        {
            wade.warn('Warning: Trying to get JSON data ' + name + ' without loading it first');
            return {};
        }
    };
    ...
    this.getText = function(name)
    {
        if (this.loadingStatus[name] == 'ok')
        {
            return this.loadedText[name];
        }
        else
        {
            wade.warn('Warning: Trying to get text data ' + name + ' without loading it first');
            return '';
        }
    };
    ...
    this.getAudio = function(name)
    {
        ...
        {
            wade.warn('Warning: Trying to get audio ' + name + ' without loading it first');
            return new Audio();
        }
    };
    ...
    this.getScript = function(name)
    {
        ...
        {
            wade.warn('Warning: Trying to get script ' + name + ' without loading it first');
            return '';
        }
    };
    ...
    this.getFont = function(name)
    {
        ...
        {
            wade.warn('Warning: Trying to get font ' + name + ' without loading it first');
            return '';
        }
    };
    ...
    this.handleLoadingCallback = function(callback, data, fileName)
    {
        ...
        if (this.pendingCallbacks[fileName] && this.pendingCallbacks[fileName].success)
        {
            for (var i=0; i<this.pendingCallbacks[fileName].success.length; i++)
            {
                this.pendingCallbacks[fileName].success[i] && this.pendingCallbacks[fileName].success[i](data, fileName);
            }
            delete this.pendingCallbacks[fileName];
        }
        if (this.globalCallback && this.isFinishedLoading())
        {
            this.globalCallback();
            this.setGlobalCallback(null);
        }
    };

    this.handleErrorCallback = function(fileName, errorCallback, message)
    {
        if (errorCallback)
        {
            errorCallback();
        ...
        {
            wade.error(message + ' ' + fileName);
        }
        if (this.pendingCallbacks[fileName] && this.pendingCallbacks[fileName].error)
        {
            for (var i=0; i<this.pendingCallbacks[fileName].error.length; i++)
            {
                this.pendingCallbacks[fileName].error[i] && this.pendingCallbacks[fileName].error[i](data, fileName);
            }
            delete this.pendingCallbacks[fileName];
        }
    };
    ...
    this.loadFont = function(name, callback, errorCallback)
    {
		...
		{
			wade.log("It isn't possible to load fonts dynamically in this environment, because it lacks support for some HTML5 features");
			this.fontLoaded(name, callback)();
			return;
		}
    ...
    this.fontLoadingError = function(name, callback, errorCallback)
    {
        ...
        return function()
        {
            ...
            if (that.attemptsCount[name] < that.maxAttempts)
            {
                ...
                {
                    wade.error('Unable to load font ' + name);
                }
