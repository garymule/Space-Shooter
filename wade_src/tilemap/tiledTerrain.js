function TiledTerrain()
{
    ...
    this.setNumTiles = function(numTilesX, numTilesY)
    {
        ...
        for (j=this.numTilesY; j<numTilesY; j++)
        {
            for (i=0; i<this.numTilesX; i++)
            {
                this.setTile(i, j, defaultTileData);
    ...
    this.setTile = function(x, y, tileData)
    {
        ...
        else
        {
            ...
            var pos = {x: this.c_origin.x + this.c_tileSize.x*x + this.c_tileSize.x/2 - this.c_tileSize.x*this.numTilesX/2,
                       y: this.c_origin.y + this.c_tileSize.y*y + this.c_tileSize.y/2 - this.c_tileSize.y*this.numTilesY/2};
            ...
            // adjust position of transitions, details
            var topPos = {x: this.c_origin.x + this.c_tileSize.x*x + this.c_tileSize.x/2 - this.c_tileSize.x*this.numTilesX/2,
                          y: this.c_origin.y + this.c_tileSize.y*y + this.c_tileSize.y/2 - this.c_tileSize.y*this.numTilesY/2};
            if (this.transitionSprites[x] && this.transitionSprites[x][y])
            {
                this.owner.setSpriteOffset(this.owner.getSpriteIndex(this.transitionSprites[x][y]), topPos);
            }
            if (this.detailSprites[x] && this.detailSprites[x][y])
            {
                this.owner.setSpriteOffset(this.owner.getSpriteIndex(this.detailSprites[x][y]), topPos);
            }
            if (this.gridSprites[x] && this.gridSprites[x][y])
            {
                this.owner.setSpriteOffset(this.owner.getSpriteIndex(this.gridSprites[x][y]), this.getWorldCoordinates(x, y));
            }
        }
    ...
    {
        var minX = this.c_origin.x - this.numTilesX/2*this.c_tileSize.x;
        var maxX = this.c_origin.x + this.numTilesX/2*this.c_tileSize.x;
        var minY = this.c_origin.y - this.numTilesY/2*this.c_tileSize.y;
        var maxY = this.c_origin.y + this.numTilesY/2*this.c_tileSize.y;
        var worldX = -this.c_tileSize.x*this.numTilesX/2 + this.c_tileSize.x/2 + tileX*this.c_tileSize.x + this.c_origin.x;
        var worldY = -this.c_tileSize.y*this.numTilesY/2 + this.c_tileSize.y/2 + tileY*this.c_tileSize.y + this.c_origin.y;
        return {x:worldX, y:worldY, valid:(valueInRange(worldX, minX, minY) && valueInRange(worldY, minX, minY))};
    };

    this.getTileCoordinates = function(worldX, worldY)
    {
        var cellX = Math.floor((worldX - this.c_origin.x + this.numTilesX*this.c_tileSize.x/2) / this.c_tileSize.x);
        var cellY = Math.floor((worldY - this.c_origin.y + this.numTilesY*this.c_tileSize.y/2) / this.c_tileSize.y);
