...
function Wade()
{
     ...
     * @param {string} file A text file to load. It can be a relative path, or an absolute path starting with "http://"
    ...
    /**
     * @param {string} file A text file to access (it has to be a file that has been loaded via wade.loadText() or set via wade.setText() first). It can be a relative path, or an absolute path starting with "http://"
     ...
     * @param {Array} [exclude] An array of scene objects and path (or their names) to exclude from the exported scene
     ...
     * @param {object} data A scene description object, such as one created with wade.exportScene(). The format is the following (all fields are optional):<ul>
     {<br/>
		  ...
		  <li><b>sceneObjects</b>: An array containing all the SceneObjects to instantiate. See the SceneObject documentation for more details about the format to use</li>
    ...
    this.importScene = function(data, loadingBar, callback, async, clearScene)
    {
        ...
        var afterLoading = function()
        {
            if (++numLoaded == numToLoad)
            {
                ...
                // load scene data through external modules
                if (data.modules)
                {
    ...
    this.setTimeout = function(f, time)
    {
        ...
        timeouts.push({f: f, time: self.getAppTime() * 1000 + time, uid: ++timeoutUid});
        return timeoutUid;
    };
    ...
    this.setInterval = function(f, time)
    {
        ...
        timeouts.push({f: f, time: self.getAppTime() * 1000 + time, repeat: time, uid: ++timeoutUid});
        return timeoutUid;
    };
...
this.clearInterval = function(intervalUid)
{
	...
	this.findPath = function(parameters)
	{
		...
		var boundaries = parameters.boundaries || {minX: 0, minY: 0, maxX: collisionMap[0].length-1, maxY: collisionMap[0][0].length-1};
